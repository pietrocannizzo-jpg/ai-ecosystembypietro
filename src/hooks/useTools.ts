      ...
      try {
        ...
        const mapped: CardData[] = (cards || []).map((c) => {
          ...
          return {
            ...
            title: c.title,
            icon: c.icon || "🔧",
            // Prefer local category/subcategory/color (handles category restructures before DB is updated)
            category: localMatch?.category || c.category,
            subcategory: localMatch?.subcategory || c.subcategory || "",
            color: localMatch?.color || c.color,
            summary: localMatch?.summary || c.summary || "",
